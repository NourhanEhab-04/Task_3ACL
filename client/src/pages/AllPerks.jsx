...
export default function AllPerks() {
  ...
  // 1️⃣ useEffect #1: Load all perks on first mount
  useEffect(() => {
    loadAllPerks()
  }, []) // Empty dependency = runs only once on mount

  // 2️⃣ useEffect #2: Auto-search when searchQuery or merchantFilter changes
  useEffect(() => {
    ...
    // Cleanup timeout if user keeps typing before 500ms passes
    return () => clearTimeout(delay)
  }, [searchQuery, merchantFilter])

  // 3️⃣ useEffect #3: Extract unique merchant list
  useEffect(() => {
